...
To edit a lesson:
1. Check out the `completed` branch.
...
1. *Make changes...*
1. When finished with your changes, run `git add .` and `git commit --amend`.
1. Continue with the rebase with `git rebase --continue`.
1. Force push to the branch (`git push --force`).
